...
object Huffman {
  ...
  def decodetemp(tree: CodeTree, root: CodeTree, bits: List[Bit]): List[Char] = tree match {
    case Fork(left, right, chars, w) =>
      if (bits.head == 0)
        decodetemp(left, root, bits.tail)
      else
        decodetemp(right, root, bits.tail)

    case Leaf(ch, w) =>
      List(ch)
      if (bits.tail.size > 0)
        decodetemp(root, root, bits.tail)
