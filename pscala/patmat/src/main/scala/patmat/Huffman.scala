...
object Huffman {
  ...
  def until(isS: List[CodeTree] => Boolean, comb: List[CodeTree] => List[CodeTree])(trees: List[CodeTree]): List[CodeTree] =
    if (isS(trees))
      trees
    else until(isS, comb)(comb(trees))
  ...
  def containsChar(tree: CodeTree, c: Char): Boolean = tree match {
    case Fork(left, right, chars, w) => chars.contains(c)
    case Leaf(ch, w) => ch == c
  }
  ...
  def encodeChar(tree: CodeTree, c: Char, bits: List[Bit]): List[Bit] = tree match {
    case Fork(left, right, chars, w) =>
      if (containsChar(left, c)) {
        encodeChar(left, c, bits :+ 0)
      } else {
        encodeChar(right, c, bits :+ 1)
      }
    case _ => bits
  }
  ...
  def encode(tree: CodeTree)(text: List[Char]): List[Bit] = {
    var bits = List[Bit]()
    for (c <- text) {
      bits = bits ::: encodeChar(tree, c, List[Bit]())
    }
    bits
  }
